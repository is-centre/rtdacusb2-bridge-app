...
namespace RTDACUSB2_0103NS
{
    class RTDACUSB2_0103 : RTDACUSB2
    {
        public RTDACUSB2_0103( )
            : base(  504, /* BufferLength  */ 
                      26, /* NoOfDigitalIO */ 
                       8, /* NoOfPWM       */ 
                       0, /* NoOfEncoder   */
                       8, /* NoOfEncoderI  */ 
        ...
        public override int ReadUSBFrame()
        {
            ...
            for (i = 0; i < NoOfFreqM; i++)
            {
                ...
                FreqM(i).Counter = (((UInt32)pUIntBuffer[108 + 2 * i]) << 16) | (UInt32)pUIntBuffer[107 + 2 * i];
                FreqM(i).Result = (((UInt32)pUIntBuffer[124 + 2 * i]) << 16) | (UInt32)pUIntBuffer[123 + 2 * i];
            }

            for (i = 0; i < NoOfChrono; i++)
            {
                Chrono(i).Enable = ChronoClass.ToEnable((pUIntBuffer[140+i] >>  0) & 0x0001);
                Chrono(i).TriggerMode = ChronoClass.ToTriggerMode((pUIntBuffer[140+i] >>  1) & 0x0003);
                Chrono(i).EnableGate = ChronoClass.ToEnableGate((pUIntBuffer[140+i] >>  4) & 0x0001);
                Chrono(i).InvertStartStop = ChronoClass.ToInvertStartStop((pUIntBuffer[140+i] >>  6) & 0x0001);
                Chrono(i).InvertStop = ChronoClass.ToInvertStop((pUIntBuffer[140+i] >>  7) & 0x0001);
                Chrono(i).InvertGate = ChronoClass.ToInvertGate((pUIntBuffer[140+i] >>  8) & 0x0001);
                Chrono(i).Next = ChronoClass.ToNext((pUIntBuffer[140 + i] >> 10) & 0x0001);
                Chrono(i).Arm = ChronoClass.ToArm((pUIntBuffer[140 + i] >> 11) & 0x0001);
                Chrono(i).BlockStatus = ChronoClass.ToBlockStatus((pUIntBuffer[140 + i] >> 13) & 0x0007);
                Chrono(i).Divider = (((UInt32)pUIntBuffer[149 + 2 * i]) << 16) | (UInt32)pUIntBuffer[148 + 2 * i];
                Chrono(i).Counter = (((UInt32)pUIntBuffer[165 + 2 * i]) << 16) | (UInt32)pUIntBuffer[164 + 2 * i];
                Chrono(i).Result = (((UInt32)pUIntBuffer[181 + 2 * i]) << 16) | (UInt32)pUIntBuffer[180 + 2 * i];
            }

            AD(0).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[200] >> 0) & 0x0007);
            AD(1).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[200] >> 4) & 0x0007);
            AD(2).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[200] >> 8) & 0x0007);
            AD(3).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[200] >> 12) & 0x0007);
            AD(4).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[201] >> 0) & 0x0007);
            AD(5).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[201] >> 4) & 0x0007);
            AD(6).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[201] >> 8) & 0x0007);
            AD(7).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[201] >> 12) & 0x0007);
            AD(8).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[202] >> 0) & 0x0007);
            AD(9).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[202] >> 4) & 0x0007);
            AD(10).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[202] >> 8) & 0x0007);
            AD(11).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[202] >> 12) & 0x0007);
            AD(12).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[203] >> 0) & 0x0007);
            AD(13).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[203] >> 4) & 0x0007);
            AD(14).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[203] >> 8) & 0x0007);
            AD(15).ADGain = ADClass.ToGain(((UInt32)pUIntBuffer[203] >> 12) & 0x0007); 
            for (i = 0; i < ADClass.NoOf; i++)
            {
                AD(i).ADValue = (UInt32)pUIntBuffer[204 + i] & 0x0FFF;
                AD(i).ADBusy = ADClass.ToBusy(((UInt32)pUIntBuffer[204 + i] >> 12) & 0x0001);
            }
...
//ADReady = (int)(((UInt32)pUIntBuffer[215] >> 1) & 0x0001);
            ...
            {
                DA(i).DAValue = (UInt32)pUIntBuffer[220 + i] & 0x3FFF;
            }
        ...
        public override int SendUSBFrame()
        {
            ...
            {
                pUIntBuffer[140 + i] = 2048 * ChronoClass.ArmToUInt(Chrono(i).Arm) +
                ...
                pUIntBuffer[148 + 2 * i] = Chrono(i).Divider & 0x7FFF;
                pUIntBuffer[149 + 2 * i] = (Chrono(i).Divider >> 15) & 0x7FFF;
            }

            for (i = 0; i < DAClass.NoOf; i++)
            {
                pUIntBuffer[220 + i] = DA(i).DAValue & 0x3FFF;
            }

            pUIntBuffer[200] = pUIntBuffer[191] = pUIntBuffer[192] = pUIntBuffer[193] = 0;
            pUIntBuffer[200] += ((ADClass.GainToUInt(AD(0).ADGain) & 0x0007) << 0);
            pUIntBuffer[200] += ((ADClass.GainToUInt(AD(1).ADGain) & 0x0007) << 4);
            pUIntBuffer[200] += ((ADClass.GainToUInt(AD(2).ADGain) & 0x0007) << 8);
            pUIntBuffer[200] += ((ADClass.GainToUInt(AD(3).ADGain) & 0x0007) << 12);
            pUIntBuffer[201] += ((ADClass.GainToUInt(AD(4).ADGain) & 0x0007) << 0);
            pUIntBuffer[201] += ((ADClass.GainToUInt(AD(5).ADGain) & 0x0007) << 4);
            pUIntBuffer[201] += ((ADClass.GainToUInt(AD(6).ADGain) & 0x0007) << 8);
            pUIntBuffer[201] += ((ADClass.GainToUInt(AD(7).ADGain) & 0x0007) << 12);
            pUIntBuffer[202] += ((ADClass.GainToUInt(AD(8).ADGain) & 0x0007) << 0);
            pUIntBuffer[202] += ((ADClass.GainToUInt(AD(9).ADGain) & 0x0007) << 4);
            pUIntBuffer[202] += ((ADClass.GainToUInt(AD(10).ADGain) & 0x0007) << 8);
            pUIntBuffer[202] += ((ADClass.GainToUInt(AD(11).ADGain) & 0x0007) << 12);
            pUIntBuffer[203] += ((ADClass.GainToUInt(AD(12).ADGain) & 0x0007) << 0);
            pUIntBuffer[203] += ((ADClass.GainToUInt(AD(13).ADGain) & 0x0007) << 4);
            pUIntBuffer[203] += ((ADClass.GainToUInt(AD(14).ADGain) & 0x0007) << 8);
            pUIntBuffer[203] += ((ADClass.GainToUInt(AD(15).ADGain) & 0x0007) << 12);
